...
//!
//! Provides background asset loading with progress tracking and error handling.
...
/// Asset load progress information
#[derive(Debug, Clone)]
...
impl AsyncAssetLoader {
        ...
        tokio::spawn(async move {
            ...
            match tokio::fs::read(&path).await {
                Ok(data) => {
                    ...
                    // Send loaded message with raw data
    ...
    /// Get load progress for an asset
    ...
    /// Get total loading progress (0.0 to 1.0)
    ...
    /// Process pending load messages
    /// This should be called regularly (e.g., in a system) to process completed loads
    pub async fn process_messages(&self, cache: &AssetCache) -> usize {
        ...
        while let Ok(msg) = receiver.try_recv() {
            match msg {
                ...
                LoaderMessage::Loaded { id, asset, path } => {
                    log::info!("Asset loaded successfully: {}", path);
                    cache.insert(id, asset);
                    processed += 1;
                }
                LoaderMessage::Failed { id: _, error } => {
                    log::error!("Asset load failed: {}", error);
                    processed += 1;
                }
            }
        }
    ...
    /// Remove progress tracking for an asset
...
/// System to process async asset loading messages
pub fn process_asset_loading(
    async_loader: Res<AsyncAssetLoader>,
    cache: Res<AssetCache>,
) {
    // Use tokio runtime to process messages
