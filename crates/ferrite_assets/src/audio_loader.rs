//! Audio asset loading.
//!
//! Loads audio files (WAV, MP3, OGG, FLAC) using symphonia.
...
pub struct AudioData {
    ...
    pub channels: u16,
    /// Audio samples (interleaved, i.e., [L, R, L, R, ...] for stereo)
    pub samples: Vec<f32>,
    /// Duration in seconds
    pub duration: f32,
}
...
impl AudioData {
    ...
    /// Create a fallback silence audio (1 second of silence)
    ...
    /// Create a simple sine wave tone for testing
    ...
    /// Get total sample count (including all channels)
...
/// Audio asset loader using symphonia
...
/// Load audio from a reader (file or memory)
fn load_audio_from_reader(
    ...
    loop {
        ...
        // Convert samples to f32 and collect
...
/// Convert various audio buffer formats to f32 samples
/// Symphonia stores audio in planar format (all samples for channel 0, then channel 1, etc.)
/// but rodio expects interleaved format (sample from each channel, then repeat).
/// This function converts planar to interleaved format.
